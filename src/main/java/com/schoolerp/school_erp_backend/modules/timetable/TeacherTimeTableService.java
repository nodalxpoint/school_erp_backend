...
public class TeacherTimeTableService {
    ...
    private TimetableDto mapToDto(TeacherTimeTableEntity entity) {
        ...
        classesRepository.findById(entity.getClassEntity().getId()).ifPresent(c -> dto.setClassName(c.getClassName()));
        sectionRepository.findById(entity.getSectionEntity().getId())
    ...
    // .findByAcademicSessionIdAndTeacher_IdAndDayOfWeek(
    ...
    // throw new ValidationException("Teacher already has a class assigned on " +
    // dayOfWeek + " Period "
    // + request.getPeriod() + ".");
    // }
    ...
    // throw new ValidationException("Teacher already has another class scheduled
    // during this time.");
    // }
    // }
    // }

    // // 2. Class + Section Conflict (Mandatory) & 6. Class Time Overlap
    // (Recommended)
    // List<TeacherTimeTableEntity> classConflicts = teacherTimeTableRepository
    // .findByAcademicSessionIdAndClassEntity_IdAndSectionEntity_IdAndDayOfWeek(
    ...
    // "Class " + classSectionDisplay + " already has a timetable assigned on " +
    // dayOfWeek
    ...
    // String requestedRoom = request.getRoomNo() != null ?
    // request.getRoomNo().trim() : "";

    // // 1. Class-to-Room Consistency: A class can only be assigned to one room on
    // a
    // // given day
    ...
    // if (existingRoomForClass != null &&
    // !existingRoomForClass.equalsIgnoreCase(requestedRoom)) {
    ...
    // + ". The incoming teacher must conduct the class in Room " +
    // existingRoomForClass + ".");
    // }

    // // 2. Room-to-Class Consistency: A room can only be assigned to one class on
    // a
    // // given day
    ...
    // if (!entry.getClassEntity().getId().equals(request.getClassId())
    // || !entry.getSectionEntity().getId().equals(request.getSectionId())) {
    // String otherClassName = "";
    // String otherSectionName = "";
    // var otherClassOpt =
    // classesRepository.findById(entry.getClassEntity().getId());
    // if (otherClassOpt.isPresent()) {
    // otherClassName = otherClassOpt.get().getClassName();
    // }
    // var otherSectionOpt =
    // sectionRepository.findById(entry.getSectionEntity().getId());
    ...
    // + " is already assigned to Class " + otherClassSectionDisplay + " on " +
    // dayOfWeek
    // + ".");
    // }
    ...
    // if (entry.getPeriod().equals(request.getPeriod())) {
    // throw new ValidationException("Room " + requestedRoom + " is already occupied
    // on " + dayOfWeek
    // + " Period " + request.getPeriod() + ".");
    // }
    // // 5. Room Time Overlap
